...
export default function CustomMetricsPage({ shared }: { shared: Shared }) {
  ...
  return (
    <Page title="自定义指标">
      <Section title="未知传感器"
        right={
          <Btn size="sm" variant="secondary" className="bg-[#27272a]" isLoading={seeding} onPress={seedPreset}>
            <Wand2 size={15} />
            一键注册默认指标集
          </Btn>
        }>
...
function UnknownForm({ sensor, onDone }: { sensor: UnknownSensor; onDone: () => void }) {
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-2">
      <Input className="w-40 font-poppins" variant="bordered" size="sm"
        placeholder="名字" value={name} onValueChange={setName} />
      <Input className="w-28 font-poppins" variant="bordered" size="sm"
        placeholder="单位，如 °C" value={unit} onValueChange={setUnit} />
      <Input className="w-20 font-poppins" variant="bordered" size="sm" type="number"
        placeholder="小数位" value={digits} onValueChange={setDigits} />
      <Checkbox isSelected={naZero} onValueChange={setNaZero} size="sm">
