...
| `multiselect` | 药丸组：点未选项 = **追加到列尾**；已选药丸带 ✕ 移除 + ↑↓ 调序；**不引入拖拽**（与 `mapping` 行同族交互，300px 窄面板里也不与框选/拖动打架） | 选项 id 的**有序**数组（顺序即语义，如表格的列序） | `choices` `default` |

`multiselect` 的定稿语义（P2 与用户拍板）：**点选追加到列尾、每颗已选药丸带 ✕ 移除 + ↑↓ 调序、不做拖拽排序**；
值数组为空 = 删键回默认（与标量控件同口径）。

扩展键语义：`ops` = mapping 的可选算子（`{id, label, needs_value}`；`needs_value=false`
的算子不填阈值，编辑器收起阈值框**并删掉 `value` 键**）；`icons` = mapping 行里可选的图标名；`choices` = multiselect 的可选项清单（`{id,label}`，如 `TABLE_KINDS`）。
...
## 七、v3 立项触发条件（P2 调查结论，下一期决策的原始材料）

P2 做表格前先实测过「v2 平铺模型能不能表达数组数据行」——**结论是能**：`cards` 早就
是一行一行的二维结构（`items` × label/value/sub/bar/spark），表格只是换一种排布。
真正的墙只有两处，且都不是数据模型：

1. **键名约定墙（可绕，也可一行破）**：版式引用遍历（`hwobs/refs.py` 的
   `REF_KEYS` / `GROUP_KEYS`）是**手写白名单元组**，不是类型系统。实测（P2 探针）：

   | 数据形状 | 引用是否收到 | 判定 |
   |---|---|---|
   | 行放 `items[]`，单元格用 `value` / `bar` / `metric` / `mapping` | 全收 | 零改动（表格就是这么做的） |
   | 纯表头/列定义放 `columns[]`（不含引用） | 不误收 | 零改动 |
   | 行数组改名 `rows[]`，或单元格容器叫 `cells{}` | **全丢** | **撞墙** |

   ⚠️ **丢引用是静默的**：`aida/controller.py` 的 `needed_ids` 按这份遍历裁剪 AIDA64
   导出清单，漏收的传感器被清掉后，OBS 里只会变成 `--`，**layout-check 零报错、界面零提示**。
   历史事故 e2e567e 就是这个形状（三份手写遍历各差一点）。
   最小破墙方案：`GROUP_KEYS += ("rows", "cells")` 一行 —— 但必须同时补一条回归测试
   锁住「新键名漏收」这类 bug，否则下次改名还是静默炸。

2. **跨部件列宽共享墙（v2 真墙）**：一个部件 = 一个宿主 DOM 子树，多个 `value` /
   `progress` 件拼不出对齐的列。所以「表格」必须在**一个部件内部**自己排 grid ——
   这点 v2 表达得下，`table` 已落地。

**本期结论：不立项。** 出现下面任一条就该立项：

- 需要任意命名的行 / 单元格容器（不再靠键名约定换表达力）；
- 需要跨部件共享列宽（两张表要对齐，或表格里嵌别的部件）；
- 需要合并单元格 / 排序 / 分页 / 行级样式等真表格语义 —— v2 的「一部件一宿主 +
  每秒全量 `update()`」模型下，行级虚拟化与增量渲染会开始难受；
- 引用遍历要从「键名约定」升级为「部件自己声明引用路径」（schema 推导）——
  这是最本质的一条，前两条都是它的症状。

v2 降级替代（不立项时的现有办法）：① 多个 `value` / `progress` 件 + 编辑器左对齐工具
拼假表格（列宽不共享，改一个数要重排一排）；② `cards` 关括号（`style.brackets=false`）
+ `cols=N` + 压扁 `item_height`（每格自己排版，**列不对齐、没表头**）；③ `chips`
（只有名字 + 值，没有条 / 状态列）。
